...
class _ManageTasksState extends State<ManageTasks> {
  ...
  Future<void> fetchTasks() async {
    try {
      final response = await http.get(
        // Uri.parse('http://localhost/PHPMailer-6.9.3/PHPMailer/tasks.php'),
        Uri.parse('http://localhost/PHPMailer-6.9.3/PHPMailer/tasks.php'),
      );
  ...
  Future<void> fetchUsers() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost/PHPMailer-6.9.3/PHPMailer/user.php'),
      );
  ...
  Future<void> createTask(
      String title, String description, String duration) async {
    try {
  ...
  Future<void> updateTask(
      int id, String title, String description, String duration) async {
    try {
      final response = await http.put(
        Uri.parse(
            'http://localhost/PHPMailer-6.9.3/PHPMailer/tasks.php?id=$id'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          "user_id": assignedToEmail,
          "title": title,
  ...
  Future<void> sendEmail() async {
    try {
      final response = await http.post(
        Uri.parse('http://localhost/PHPMailer-6.9.3/PHPMailer/sendemail.php'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({"recipient": emailsend, "message": "Hello, $desc "}),
      );
              ...
              DropdownButton<String>(
                ...
                onChanged: (String? newValue) {
                  setState(() {
                    ...
                    emailsend = users.firstWhere(
                        (user) => user['id'].toString() == newValue)['email'];
                  });
                },
